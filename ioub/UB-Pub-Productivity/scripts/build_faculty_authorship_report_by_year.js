...
const outputXlsx = path.join(
  ...
  "data",
  "derived",
  "faculty_authorship_report_by_year.xlsx"
);
...
function writeMinimalXlsx(outputPath, headers, rows) {
  const buildDir = path.join(path.dirname(outputPath), "tmp_faculty_authorship_year_xlsx");
...
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
  <sheets>
    <sheet name="SummaryByYear" sheetId="1" r:id="rId1"/>
  </sheets>
</workbook>`,
    "utf8"
  );
...
function ensureSummary(summaryMap, facultyCode, facultyName, year) {
...
function main() {
  ...
  const facultyMeta = new Map();
  for (const faculty of faculties) {
    facultyMeta.set(normalize(faculty.faculty_code), normalize(faculty.faculty_name));
  }
  ...
  try {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      const year = normalize(cellMap.get(headers.get("Year")));
      ...
      for (const [code, name] of firstFaculties.entries()) {
        ensureSummary(summary, code, name || facultyMeta.get(code) || code, year).paper_count_first_author += 1;
      }

      for (const [code, name] of corrFaculties.entries()) {
        ensureSummary(summary, code, name || facultyMeta.get(code) || code, year).paper_count_corresponding_author += 1;
      }

      const eitherFaculties = new Map([...firstFaculties, ...corrFaculties]);
      for (const [code, name] of eitherFaculties.entries()) {
        const row = ensureSummary(summary, code, name || facultyMeta.get(code) || code, year);
        row.paper_count_first_or_corresponding_author += 1;
        if (firstFaculties.has(code) && corrFaculties.has(code)) {
          row.paper_count_both_first_and_corresponding_author += 1;
        }
      }
    }
